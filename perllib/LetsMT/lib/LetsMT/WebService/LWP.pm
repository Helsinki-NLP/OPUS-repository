...
sub user_agent {
    ...
    ## return existing user agent if it exists
    return $UserAgents{$server} if (exists $UserAgents{$server});
...
sub storage_url {
    ...
    get_logger(__PACKAGE__)->debug("get user and server ...");
    ...
    get_logger(__PACKAGE__)->debug("return URL $url ...");
...
sub get {
    my $fun = \&storage_url;
    get_logger(__PACKAGE__)->debug("get request to storage ...");
